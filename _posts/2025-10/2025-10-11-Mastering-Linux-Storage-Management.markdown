...
- **Tools Used**: `mdadm`, `parted`, `wipefs`, `mkfs.ext4`, `dd`, `tar`, `fallocate`, `lsof`, `lsblk`, `blkid`, `partprobe`, `e2fsck`, `resize2fs`, `lvextend`, `lvreduce`
...
- **Filesystem Management** (Questions 23, 24, 29, 30-32, 33, 66, 83, 90, 94, 96, 100): Partitioning, formatting, mounting, and cleanup.
...
2. **Partition Disk**:
   ...
   - **Command**: `parted /dev/vdc mklabel gpt mkpart primary ext4 1MiB 100%; parted /dev/vdc set 1 lvm on; partprobe /dev/vdc`
...
5. **Create Logical Volume (LV)**:
   ...
   - **Command**: `lvcreate -L 7G -n my_lv my_vg`
   - **Purpose**: Create a 7GB LV named `my_lv` in `my_vg` to allow reduction (Question 25).
...
8. **Reduce LV**:
   ...
   - **Command**: `umount /mnt/test; e2fsck -f /dev/my_vg/my_lv; resize2fs /dev/my_vg/my_lv 4G; lvreduce -L 4G /dev/my_vg/my_lv; mount /dev/my_vg/my_lv /mnt/test`
...
9. **Remove LV, VG, PV**:
   ...
   - **Good Practice**: Verify VG/PV removal with `vgs`, `pvs`, `blkid`.
...
1. **Wipe and Partition Disk**:
   ...
   - **Command**: `wipefs -a /dev/vdb; parted /dev/vdb mklabel gpt mkpart primary ext4 1MiB 100%; partprobe /dev/vdb`
...
6. **File Archive with `tar`**:
   ...
   - **Bookmark**:
     ...
     - Use `cd <dir>; tar -czvf <archive> .` or `--strip-components` to avoid nested paths (Question 81).
...
8. **Partition Table Backup**:
   ...
   - **Good Practice (GP14)**: Use `dd` for partition table backups; verify with `hexdump -C`.
...
1. **Simulate Full Filesystem**:
   ...
   - **Bookmark (B38)**: Simulate full filesystem with `dd`; check with `df -h` (Question 34).
   - **Good Practice (GP19)**: Monitor disk usage with `df -h` during large operations.
...
4. **Free Space**:
   ...
   - **Bookmark**:
     - Free held files with `lsof` and `kill -9` (Question 34).
     - `tail -f &` holds files; use `lsof` and `kill -9` (Question 34).
   - **Good Practice (GP17)**: Use `lsof | grep deleted` and `kill -9` to free space.
...
1. **Unmount Filesystems**:
   ...
   - **Good Practice (GP20)**: Unmount filesystems at lab end; verify with `df -h`.
...
2. **Disable Swap**:
   ...
   - **Good Practice (GP20)**: Disable swap at lab end.
...
3. **Remove Test Files**:
   ...
   - **Good Practice (GP20)**: Remove test files at lab end.
...
4. **Clean `/etc/fstab`**:
   ...
   - **Good Practice (GP20)**: Verify `/etc/fstab` at lab end.
...
5. **Wipe Disks**:
   ...
   - **Bookmark (B12)**: Use `wipefs -a` to clear metadata (Questions 30, 32).
   - **Good Practice (GP12)**: Use `wipefs -a`; verify with `blkid`, `lsblk`.
...
6. **Final Verification**:
   ...
   - **Bookmark (B42)**: Verify lab completion with `lsblk`, `blkid`, `/etc/fstab`, snapshots (All Questions).
   - **Good Practice (GP20)**: Verify with `lsblk`, `blkid`, `df -h` at lab end.
...
- **GP8**: Run `resize2fs` after `lvextend` or before `lvreduce`; verify with `df -h` (Section 1).
